...
const downgradeChirpyRed = `-- name: DowngradeChirpyRed :one
UPDATE users
...
func (q *Queries) DowngradeChirpyRed(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRowContext(ctx, downgradeChirpyRed, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
